...
function loginByIdentity(identity) {
  ...
  putEmployeeSession_(token, { name:'', surname:'', pesel, plant: plantOptions[0] || '' });
...
function putEmployeeSession_(token, sess) {
  CacheService.getScriptCache().put(
    `sess:${token}`,
    JSON.stringify({
      name: safe_(sess && sess.name),
      surname: safe_(sess && sess.surname),
      pesel: safe_(sess && sess.pesel),
      plant: safe_(sess && sess.plant)
    }),
    CFG.SESSION_TTL_SEC
  );
}
...
function saveEmployeeForm(payload) {
  ...
  if (!sessRaw) throw new Error('Sesja wygasła. Zaloguj się ponownie.');
  ...
  // Odśwież ważność sesji (sliding TTL), żeby autozapis i dłuższe wypełnianie formularza nie zrywało pracy.
  putEmployeeSession_(payload.token, { name, surname, pesel, plant });
